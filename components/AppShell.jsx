    ...
    <div className="flex h-screen bg-background text-on-surface overflow-hidden">
      ...
      <div className="fixed top-0 inset-x-0 z-30 lg:hidden bg-surface/80 backdrop-blur-lg border-b border-white/10">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsMobileMetricsOpen((prev) => !prev)}
              className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-on-surface-variant hover:text-primary transition-colors px-2 py-1 rounded-md hover:bg-primary/10"
              aria-expanded={isMobileMetricsOpen}
              aria-controls="mobile-metrics-panel"
            >
              Métricas
              <svg
                className={`w-3 h-3 transition-transform duration-200 ${isMobileMetricsOpen ? 'rotate-180' : 'rotate-0'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pt-0 pt-14">
        <div
          ...
          className={`lg:hidden border-b border-white/10 bg-surface/70 backdrop-blur-lg transition-all duration-300 overflow-hidden ${isMobileMetricsOpen ? 'max-h-[520px] opacity-100' : 'max-h-0 opacity-0'}`}
        >
          <MetricsPanel metrics={metrics} mode="mobile" />
        </div>
      ...
      <MetricsPanel metrics={metrics} mode="desktop" />
    </div>
  );
}